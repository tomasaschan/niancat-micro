...
class NineLetterDictionary(strings: Seq[String]) extends Dictionary {
  ...
  val answers: Map[String, Seq[String]] = dictionary.toSeq map (_.letters) groupBy sortByCodePoints
  ...
  override def solutions(puzzle: Puzzle): Seq[Word] =
    answers getOrElse (sortByCodePoints(puzzle.letters), Seq()) map Word

  override def solutionId(puzzle: Option[Puzzle], word: Word): Option[Int] = {
    puzzle
      .map(solutions)
      .map(_ indexOf (word.norm.letters))
      .filter(_ != -1)
      .map(_ + 1)
  }
